...
export class WebSocketNetworkProvider {
    ...
    connectSocket(onReady) {
        ...
        this.connectionTimeout = setTimeout(() => {
            if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
                this.manager.notifyStatus(`[Online 1] Could not connect to Server (${url}). Please ensure the server is running!`, true);
                this.disconnect();
            }
        }, 10000);
        ...
        this.ws.onmessage = (event) => {
            ...
            if (type === 'ROOM_CREATED') {
                ...
                this.manager.notifyStatus(`[Online 1] Room [${this.roomCode}] ready! Waiting for players...`);
                ...
                this.manager.notifyStatus(`[Online 1] ✅ Connected to room [${this.roomCode}]! (Slot ${this.slotIndex + 1})`);
                ...
                this.isConnected = true;
                this.manager.notifyStatus(`[Online 1] ✅ New player joined the room!`);
                this.manager.notifyConnected(this.role, this.roomCode, this.slotIndex, this.roomMode);
                ...
                this.manager.notifyStatus(`[Online 1] ⚠️ ${packet.message || 'Player has left the room!'}`, true);
    ...
    joinRoom(rawCode) {
        ...
        this.connectSocket(() => {
            if (this.ws && this.ws.readyState === WebSocket.OPEN) {
                this.manager.notifyStatus(`[Online 1] Searching and joining room [${this.roomCode}]...`);
                this.ws.send(JSON.stringify({
                    type: 'JOIN_ROOM',
                    roomCode: this.roomCode
                }));
...
export class WebRTCNetworkProvider {
    ...
    createRoom() {
        ...
        try {
            if (typeof Peer === 'undefined') {
                this.manager.notifyStatus('[Online 2] Error: PeerJS library is not loaded!', true);
                return;
            }
            ...
            this.manager.notifyStatus(`[Online 2] Failed to initialize room: ${e.message}`, true);
        }
    }
    ...
    joinRoom(rawCode) {
        ...
        this.manager.notifyStatus(`[Online 2] Searching room [${code}] via WebRTC...`);
        ...
        try {
            ...
            this.peer.on('open', () => {
                this.startHeartbeat();
                this.manager.notifyStatus(`[Online 2] Found room [${code}]! Performing STUN/TURN handshake...`);
            ...
            this.peer.on('error', (err) => {
                ...
                if (err.type === 'peer-unavailable') {
                    this.manager.notifyStatus(`[Online 2] Room [${code}] does not exist or has been closed!`, true);
                } else {
                    this.manager.notifyStatus(`[Online 2] Connection error: ${err.type || err.message}`, true);
                }
            });
    ...
    setupConnection(conn, isHost) {
        ...
        if (!isHost) {
            this.connectionTimeout = setTimeout(() => {
                if (!this.isConnected) {
                    this.manager.notifyStatus(`[Online 2] ⚠️ Connection timed out (20s). You may try ONLINE 1 (WebSocket) mode instead!`, true);
                    this.disconnect();
                }
            }, 20000);
        }
